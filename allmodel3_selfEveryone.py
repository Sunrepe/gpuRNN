...
df_save_path = 'E:/Research-bachelor/storeMODELs/allmodel3_self/allmodel3_selfeveryone_precision_kfold{}.csv'.format(k_fold_num)
df_save_path2 = 'E:/Research-bachelor/storeMODELs/allmodel3_self/allmodel3_selfeveryone_recall_kfold{}.csv'.format(k_fold_num)
df_save_path3 = 'E:/Research-bachelor/storeMODELs/allmodel3_self/allmodel3_selfeveryone_f1score_kfold{}.csv'.format(k_fold_num)
...
def Matrix_to_CSV_a(filename, data):
    with open(filename, "a", newline='', ) as csvfile:
        writer = csv.writer(csvfile)
        # 先写入columns_name
        # writer.writerow(["emg1", "emg2", "emg3", "emg4", "emg5", "emg6", "emg7", "emg8", "label"])
...
def main():
    ...
    test_sets = All_data_merge_self(foldname=foldname, max_seq=max_seq,
    ...
    for i_pre in range(9):
        ...
        precision_[1].append(_percision)
        precision_[2].append(_recall)
        precision_[3].append(_f1Score)
        # matrix
    ...
    __a = np.array([result_labels, predictions]).T
    Matrix_to_CSV(pre_res, __a)
    _percision = metrics.precision_score(result_labels, predictions, average="weighted")
    _recall = metrics.recall_score(result_labels, predictions, average="weighted")
    _f1Score = metrics.f1_score(result_labels, predictions, average="weighted")
    precision_[1].append(_percision)
